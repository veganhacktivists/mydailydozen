    ...
    <div class="relative py-16 bg-white overflow-hidden">
        ...
        <div class="relative px-4 sm:px-6 lg:px-8">
            <div class="text-lg max-w-prose mx-auto mb-6">
                <p class="text-base text-center leading-6 text-teal-600 font-semibold tracking-wide uppercase">More About</p>
                <h1 class="mt-2 mb-8 text-3xl text-center leading-8 font-extrabold tracking-tight text-gray-900 sm:text-4xl sm:leading-10">{{ $group->name }}</h1>
            </div>
            <div x-data="{ metric: true }" class="prose prose-lg text-gray-500 mx-auto">
                <figure>
                    <img class="w-full rounded-lg" src="{{ asset($group->banner_location) }}" alt="{{ $group->name }}" width="1310" height="873">
                ...
                <div class="btn-group btn-group-toggle inline" data-toggle="buttons">
                    <span class="inline-flex rounded-md shadow-sm">
                      <button x-on:click="metric = false" class="inline-flex items-center px-2.5 py-1.5 border border-transparent text-xs leading-4 font-medium rounded text-white bg-teal-600 hover:bg-teal-500 focus:outline-none focus:border-teal-700 focus:shadow-outline-teal active:bg-teal-700 transition ease-in-out duration-150">
                        Imperial
                      </button>
                    </span>
                    <span class="inline-flex rounded-md shadow-sm">
                      <button x-on:click="metric = true" class="inline-flex items-center px-2.5 py-1.5 border border-transparent text-xs leading-4 font-medium rounded text-white bg-teal-600 hover:bg-teal-500 focus:outline-none focus:border-teal-700 focus:shadow-outline-teal active:bg-teal-700 transition ease-in-out duration-150">
                        Metric
                      </button>
                    </span>
                </div>
