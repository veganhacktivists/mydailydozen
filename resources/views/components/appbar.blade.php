    ...
    <div class="ml-4 flex items-center md:ml-6">
      ...
      <div class="ml-3 relative" x-data="{ closed: true }" @click="closed = !closed">
        <div>
          <button
            ...
            <img class="h-8 w-8 rounded-full" src="{{ auth()->user()->profile_photo_url }}"
              alt="{{ auth()->user()->name }}">
            <p class="hidden ml-3 text-cool-gray-700 text-sm leading-5 font-medium lg:block">{{ auth()->user()->name }}
            </p>
        ...
        <div class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg">
          <div :class="{ 'hidden': closed }" class="py-1 rounded-md bg-white shadow-xs" role="menu"
